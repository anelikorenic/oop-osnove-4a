...
class ImenikApp:
    ...
    def dodaj_kontakt(self):
        ...
        if not (ime and email and telefon):
            messagebox.showwarning("TW", "Nešto ti nedostaje!")
            return
    ...
    def spremi_kontakte(self):
        ...
        messagebox.showinfo("Spremno", f"stored u {os.getcwd()}")
        self.root.destroy()
    ...
    def obrisi_kontakt(self):
        ...
        if not selekcija:
            messagebox.showwarning("TW", "Nisi ništa odabrao/la.")
            return

        indeks = selekcija[0]
        self.kontakti.pop(indeks)
        self.osvjezi_prikaz()
        messagebox.showinfo("Delete", "Uspješno si obrisao/la kontakt.")
